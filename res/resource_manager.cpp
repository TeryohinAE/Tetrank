																	  ...
																	  const std::string& fragment_path)
{
	...
	std::shared_ptr<Render::Shader_Program> &new_shader =
		shader_program_map.emplace(shader_program_name, std::make_shared<Render::Shader_Program>(vertex_path, fragment_path)).first->second;
...
{
	std::ifstream f;
	f.open(res_path + path_symbol.c_str() + relative_file_path.c_str(), std::ios::in | std::ios::binary);
	if (!f.is_open()) {
